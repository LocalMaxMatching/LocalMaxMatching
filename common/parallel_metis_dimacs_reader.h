							   ...
							   WeightType (*post_process_function)(const WeightType edge_weight, const WeightType n1_weight, const WeightType n2_weight))
{
	...
	for(EdgeIDType i=0; i<edge_container.size(); i++)
	{
		...
		w1 = node_weights.at(edge_container[i].n1);
		w2 = node_weights.at(edge_container[i].n2);
								   ...
								   ContainerType &edge_container)
{
	...
	for(NodeIDType n=0; n<edge_container.size(); n++)
	{
		...
		{
			int partner = proc_id_of_vertex(n1, num_vertices, procs);

			msgs_for_proc[partner].push_back(edge_container[n]);
		}
		else if(n2<first_global_vertex || n2>=end_global_vertex)
		{
			int partner = proc_id_of_vertex(n2, num_vertices, procs);
					  ...
					  const bool verbose = false)
{
	...
	NodeIDType _num_local_vertices = base_size + (proc_id < (int)(num_vertices % procs));

	first_global_vertex = proc_id*base_size + (proc_id < (int)(num_vertices % procs) ? proc_id : (num_vertices % procs));
	end_global_vertex  = first_global_vertex + _num_local_vertices;
...
//	std::vector<WeightType> node_weights(num_vertices, 1);
	...
	while(infile.good() && ( (no_index_file && src_vertex<num_vertices) || (!no_index_file && src_vertex<end_global_vertex) ) )
	{
		infile.getline(line);
	...
	while(infile.good() && ( (no_index_file && src_vertex<num_vertices) || (!no_index_file && src_vertex<end_global_vertex) ) )
	{
		...
		{ // not a comment-line
			...
			if(src_vertex>=first_global_vertex && src_vertex<end_global_vertex)
			{
				...
				{
					if(e_it->n2>=first_global_vertex && e_it->n2<end_global_vertex)
					{
	...
	if(! no_index_file)
	{
		fill_node_weights_from_file<WeightType, NodeIDType, EdgeIDType, Edge>(index_filename.str(), edge_container, node_weights);
	}
	...
	send_cross_edges_to_neighbors<NodeIDType, Edge, ContainerType>(first_global_vertex, end_global_vertex,
								  num_vertices, edge_container);
#endif

	proc_ranges.resize(procs);

	// set the node range of each process
	for(int p=0; p<procs; p++)
	{
		NodeIDType range_size = base_size + (p < (int)(num_vertices % procs));
		NodeIDType start = p*base_size + (p < (int)(num_vertices % procs) ? p : (num_vertices % procs));

		proc_ranges[p] = std::make_pair(start, start+range_size);
	}
